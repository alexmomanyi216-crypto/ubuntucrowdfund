...
export default function FuneralCampaign() {
	...
	}, [endDate, campaign.raised])

	async function handleDonate({ donorName, amount, paymentMethod }) {
		const res = await Api.createDonation({ donorName, amount, paymentMethod })
		setCampaign((c) => ({ ...c, raised: c.raised + amount }))
		setDonors((d) => [res.donation, ...d])
	}
							...
							<section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
								...
								<div className="mt-3 grid grid-cols-3 gap-2 text-center">
									<div className="rounded-lg bg-gray-100 p-3">
										...
										<p className="text-sm font-semibold text-gray-900">${campaign.raised.toLocaleString()}</p>
									</div>
									<div className="rounded-lg bg-gray-100 p-3">
